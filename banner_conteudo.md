...
O projeto explora um alto nível de complexidade arquitetural, baseando-se nos seguintes conceitos e tecnologias:
* **Múltiplos Agentes:** Interação orquestrada entre três papéis (Guardião, Ingênuo e Juiz Oculto).
* **APIs em Nuvem e Failover:** A aplicação é hospedada localmente via **FastAPI**, consumindo APIs de IA em nuvem (Groq, Gemini, OpenRouter) com sistema de rotação de chaves.
* **Técnicas de RAG:** Processamento de uploads de PDF (`PyMuPDF`) para extração, higienização e injeção em tags estruturadas `<document_content>`.
* **Persistência de Dados:** Banco de dados **SQLite** para log de auditoria, sessões e pontuação (*Leaderboard*).

O sistema conta com camadas de defesa especializadas:
1. **Pipeline de PDFs**: Valida binários, extrai textos isolados, sanitiza e insere contexto delimitado.
2. **Juiz Oculto**: Um LLM autônomo (*LLM-as-a-Judge*) que audita as respostas buscando vazamentos.
3. **Filtro e Sanitização (A Ausência do LLM Firewall):** Na camada de entrada, optou-se por **Filtros Determinísticos (Expressões Regulares)** em vez de um **LLM Firewall**. Essa decisão garante **baixíssima latência** e **redução de custos**, requisitos essenciais para a fluidez do jogo em tempo real. Além disso, expor o Guardião diretamente ao ataque (mitigado por Regex) exalta a eficácia do *Prompt Engineering* defensivo em seu núcleo.
